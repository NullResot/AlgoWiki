...
> 说明：本文依据当前代码中的 `backend/wiki/urls.py`、`backend/wiki/views.py` 和序列化字段整理。现阶段这些接口主要服务 AlgoWiki 前端和管理后台，本文不是正式对外开放 API 的承诺，也不包含任何服务器地址、密钥、账号、真实域名或隐私数据。对外长期开放前，建议先增加 `/api/v1/`、访问令牌管理、调用频率说明和兼容性策略。
...
本文只记录接口形态和权限边界，不应写入以下内容：

- 服务器 IP、SSH 用户名、数据库地址、镜像仓库真实地址。
- `.env` 中的任何配置值。
- AccessKey、Secret、Token、密码、短信服务密钥、对象存储密钥。
- 真实用户的手机号、邮箱、实名信息、身份证件信息。
- 删除内容归档、审核日志、安全日志中的真实内容。

对第三方开放时，只建议开放公开内容的只读接口，例如赛事日程、赛事公告、补题链接、Wiki 已发布文章、公告、友情链接和团队成员。账户、验证码、手机号验证、实名、管理后台、审计日志、删除归档、图片上传、AI 管理配置等接口默认不对第三方开放。
...
本节接口涉及账号、邮箱、手机号、登录态或个人安全信息，仅供站内前端使用，不建议作为第三方开放 API。
...
开放 API 不一定立刻产生固定费用，但会增加服务器、带宽、数据库、缓存、日志和运维成本。成本大小取决于调用量、缓存策略和是否允许批量抓取。

可能增加的开销：

| 类型 | 说明 |
| --- | --- |
| 带宽 | 高频读取赛事列表、Wiki 正文、图片链接会消耗公网流量。 |
| 服务器 CPU 和内存 | 搜索、分页、复杂筛选会增加 Django 和数据库负载。 |
| 数据库 | 大量列表查询会增加 MySQL 连接数和查询压力。 |
| 日志存储 | API 访问日志、安全日志、限流日志会占用磁盘。 |
| CDN | 如果 API 或静态资源经 CDN，访问量上升会增加流量费用。 |
| 安全运维 | 需要处理爬虫、滥用、撞库、刷接口和异常流量。 |

降低成本的建议：

1. 优先只开放只读公开数据。
2. 对外 API 单独加 `/api/v1/`，不要直接承诺内部接口长期兼容。
3. 增加独立只读 Token，不复用站内登录 Token。
4. 对第三方调用设置按 IP 和 Token 的频率限制。
5. 对赛事公告、日程、补题链接等低频变化数据使用缓存。
6. 禁止第三方调用上传、验证码、登录、手机号验证、实名、管理和审计接口。
7. 对大列表限制最大分页深度，必要时提供增量同步字段。
8. 明确版权、缓存周期、署名要求和禁止批量抓取条款。

如果要把 AlgoWiki API 正式提供给他人，建议先完成以下工程工作：
